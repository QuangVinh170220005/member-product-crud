	...
	<?php 
		...
		if (isset($_POST['add'])) {
			...
			if (empty($_POST['title'])) {
				$err_title = 'Hãy nhập tên sản phẩm';
			}

			if (empty($_POST['price'])) {
				$err_price = 'Hãy nhập giá';
			}
			if (!empty($_FILES['image']['name']))
				{

				if ($_FILES['image']['size'] > 1024 * 1024) {
					$err_image = 'File phải nhỏ hơn 1MB';
				} else {
					$name = $_FILES['image']['name'];
					$arr = explode('.', $name);

					if ($arr[1] != 'jpg' && $arr[1] != 'png' && $arr[1] != 'jpeg') {
						$err_image = 'Chỉ được upload jpg, png, jpeg';
					} 
				}
			}

			if ($err_title == '' && $err_price == '' && $err_image == '') {
				...
				$sql_edit = "UPDATE products 
							SET title = '$title', price = '$price', image = '$image'
							WHERE id = $id";

				if ($result = $conn->query($sql_edit)) {
					...
					<div class="blog-post-area">
						...
						<div class="signup-form">
							<form action="" method="post" enctype="multipart/form-data">
								...
								<img src="<?php echo $row['image'];?>" alt="" width="80" style="margin-bottom: 15px;">
